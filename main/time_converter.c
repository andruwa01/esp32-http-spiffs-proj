...
void time_converter(time_t *time_to_convert, char *format_to_convert, char *buffer_for_converted_string, int size_of_buffer){
        ...
        ESP_LOGI(converter_tag, "Start to convert time %d with format: %s", (int)time_to_convert, format_to_convert);

        struct tm time_buffer;
        localtime_r(time_to_convert, &time_buffer);

        #ifdef VORONEZH_RUSSIA
        ...
        ESP_LOGI(converter_tag, "Finish converting process, result: %s", buffer_for_converted_string);
}